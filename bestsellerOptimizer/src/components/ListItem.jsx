...
const ListItem = ({bookID, image, audiobookLength, addBookToCalculator, index, readingSpeed, dailyHours, bookName, bookAuthor, bookAmazonLink}) => {
  ...
  }, [bookID, image, audiobookLength, index, readingSpeed, dailyHours, bookName, bookAuthor, bookAmazonLink]);
    ...
    <div className={`flex flex-col justify-start h-auto min-h-60 w-60 mr-2 bg-gray-100 ${(listIndex === 0) ? 'border-r' : 'border-x'} border-black`}>
      ...
      <div className='flex flex-row justify-start h-20 border-b border-r border-black bg-gray-200 w-60'>
        ...
        <div className='flex flex-row justify-center items-center w-1/5'>
          <IoIosAddCircle onClick={(e) => addBook(e)} className='w-5/6 h-3/4 hover:w-[90%] hover:h-[90%] text-green-700 rounded-lg cursor-pointer'/>
        </div>
      </div>

      <div className='flex flex-row justify-center items-center h-62 w-60 bg-sky-700 border-r border-black'>
        <div className='h-60 w-40'>
          <img className='h-full w-full' src={bookCover} alt={`${name}`}></img>
        </div>
      </div>
