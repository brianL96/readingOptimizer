...
const ListContainer = ({bookInfoArray, listTitle, newestDate, oldestDate, loadNextBatch, addBookToCalculator, readingSpeed, dailyHours, addLoader, containerRef}) => {
    ...
    function loadBooks(){
      ...
      infoList.forEach((element, index) => {
        //console.log(element);
        arr.push(
          <ListItem image={element.image} audiobookLength={element.length} addBookToCalculator={addBookToCalculator} index={index} readingSpeed={readingSpeed} dailyHours={dailyHours} bookName={element.title} bookAuthor={element.author}/>
        );
      });
