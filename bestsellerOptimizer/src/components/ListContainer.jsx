...
const ListContainer = ({bookInfoArray, listTitle, newestDate, oldestDate, loadNextBatch, addBookToCalculator, readingSpeed, dailyHours, addLoader, containerRef}) => {
    ...
    function loadBooks(){
      ...
      infoList.forEach((element, index) => {
        ...
        console.log(element);
        arr.push(
          <ListItem image={element.image} audiobookLength={element.length} addBookToCalculator={addBookToCalculator} index={index} readingSpeed={readingSpeed} dailyHours={dailyHours} bookName={element.title} bookAuthor={element.author} bookAmazonLink={element.amazonLink}/>
        );
      });
    ...
    function scrollToLoader(){
      let loaderElement = document.querySelector("#loaderCard");
    ...
    function calculateLastWeek(){
      ...
      let start = Date.parse(`${endDate}T00:00:00`);
  ...
  return (
    <div ref={containerRef} className='flex flex-col justify-start h-auto min-h-64 max-w-200 border-black border bg-amber-900'>
      <div className='flex flex-row justify-start w-full h-20 border-b border-b-black'>
        ...
        <div className='flex flex-row justify-end items-center h-full w-1/5'>
          <FaArrowRightLong className='h-7 w-28 text-white cursor-pointer' onClick={(e) => checkLastList(e)}/>
        </div>
      </div>
