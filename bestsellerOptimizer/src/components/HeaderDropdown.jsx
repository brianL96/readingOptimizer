...
const HeaderDropdown = ({title, items, defaultValue, changeFunction, changeHeadersOpen, headerOpen, index}) => {
    ...
    function getList(){
        ...
        if(list.length > 0){
            arr.push(
                <div className='flex flex-row justify-center items-center w-full h-8 text-black bg-gray-200'>
                    <h3>{listTitle}</h3>
                </div>
            );
        }
        ...
        list.forEach((element) => {
            arr.push(
                <div value={element.value} onClick={(e) => {changeSelectedValue(e, element.display, element.value)}} className='flex flex-row justify-start items-center w-full h-8 bg-white text-black hover:bg-gray-700 hover:text-white cursor-pointer'>
